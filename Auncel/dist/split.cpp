...
int main(int argc,char **argv) {
    ...
    int ty = std::stoi(type);
    int f1 = 2*ty;
    int f2 = 2*ty + 1;
    std::string file1 = file_dir + std::to_string(f1) + "db.fbin";
    std::string file2 = file_dir + std::to_string(f2) + "db.fbin";
    int64_t n,d;
    double t0 = elapsed();
    ...
    fbin_write(file2.c_str(), x + int64_t(n/2) * int64_t(d), d, n/2);
    delete[] x;
    return 0;
}