...
void fbin_check(const char* fname, int64_t index){
    FILE* f = fopen(fname, "r");
    if (!f) {
        fprintf(stderr, "could not open %s\n", fname);
        perror("");
...
int main(int argc,char **argv) {
    if(argc - 1 != 2){
        printf("You at least need tow params: input file and check num\n");
        return 0;
    }
    std::string file1 = argv[1];
    std::string p2 = argv[2];
    int64_t index = atoll(p2.c_str());
    printf("Index %lld\n", index);
    fbin_check(file1.c_str(), index);
    // fbin_check(file2.c_str(), index);
    return 0;
}