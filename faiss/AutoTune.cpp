...
double IntersectionCriterion::evaluate(const float* /*D*/, const idx_t* I)
    ...
    // Find the worst case
    for (idx_t q = 0; q < nq; q++) {
        long tmp = ranklist_intersection_size (
             R, &gt_I [q * gt_nnn],
             R, I + q * nnn);
        n_ok = std::min(n_ok, tmp);
    }
    return n_ok / double (R);
}
                              ...
                              OperatingPoints * ops) const
{
    ...
    if (n_experiments == 0) {
        ...
        if (DC (IndexIVF)) {
            std::string maxcom = "nprobe=" + std::to_string(ix->nlist);
            ops->add (1.0, 1e9, maxcom, n_comb);
        }
        return;
    }
